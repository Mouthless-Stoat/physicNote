...
Force that a act on an object without touching it
...
k is the spring constant (this value varies depending on the spring's material).  
...
Normal force can be calculated using the weight formula:

\\[ N = gm \\]
or on earth it simply
\\[ N = 9.8m \\]

N is the normal force (N) (also the weight)  
...
I is impulse (Ns)  
m is the object's mass (kg)  
\\(v_f\\) is the object's final velocity (m/s)  
